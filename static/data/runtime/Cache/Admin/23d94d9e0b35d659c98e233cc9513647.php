...
<html>
	...
	<style type="text/css">
		...
		.list-dot li a.close,.list-dot-othors li a.close{ background: url("/myweb/static/images/cross.png") no-repeat left 3px; display:block; width:16px; height:16px;position: absolute;outline:none;right:5px; bottom:5px}
	...
	<div class="wrap J_check_wrap">
		<div class="nav">
			<ul class="cc">
				<li class="current"><a href="#"><?php echo ((isset($term["name"]) && ($term["name"] !== ""))?($term["name"]):'页面编辑'); ?></a></li>
			</ul>
		</div>
		<form name="myform" id="myform" action="<?php echo U(CONTROLLER_NAME.':'.ACTION_NAME);?>" method="post" class="J_ajaxForms" enctype="multipart/form-data">
			<div class="col-right">
				<div class="table_full">
					<table width="100%">
							...
							<td>
								<div  style="text-align: center;"><input type='hidden' name='smeta[thumb]' id='thumb' value="<?php echo ((isset($smeta["thumb"]) && ($smeta["thumb"] !== ""))?($smeta["thumb"]):''); ?>">
									<a href='javascript:void(0);' onclick="flashupload('thumb_images', '附件上传', 'thumb', thumb_images, '1,jpg|jpeg|gif|png|bmp,1,,,1', 'content', '12', 'b6ba209759e147124653ac77362ef2bd');
											return false;">
										<img src= <?php if($smeta["thumb"] != ""): ?>"<?php echo ($smeta["thumb"]); ?>"<?php else: ?>'/myweb/static/images/icon/upload-pic.png'<?php endif; ?>" id='thumb_preview' width='135' height='113' style='cursor:hand' /></a>
									<!-- <input type="button" class="btn" onclick="crop_cut_thumb($('#thumb').val());return false;" value="裁减图片">  -->
									<input type="button"  class="btn" onclick="$('#thumb_preview').attr('src', '/myweb/static/images/icon/upload-pic.png');
											$('#thumb').val('');
						...
						<tr>
							<td>
						<?php $comment_yes=$post[comment_status]==1?"checked":""; $comment_no=$post[comment_status]==0?"checked":""; ?>
						<label class="ib" style="width:88px"><input type="radio" name='post[comment_status]'  id="allow_comment_1"  value="1" <?php echo ($comment_yes); ?>> 允许评论</label>
						<label class="ib" style="width:88px"><input type="radio" name='post[comment_status]'  id="allow_comment_0"  value="0" <?php echo ($comment_no); ?>> 不允许评论</label>
						</td>
						</tr>
						<tr>
							<td><b>微信</b></td>
						...
						<tr>
							<td>
								<span class="switch_list cc">
									<?php $wx_yes=$post[wx_status]==1?"checked":""; $wx_no=$post[wx_status]==0?"checked":""; ?>
									<label><input type="radio" name="post[wx_status]" value="1" <?php echo ($wx_yes); ?>/><span>微信显示</span></label>
									<label><input type="radio" name="post[wx_status]" value="0" <?php echo ($wx_no); ?>/><span>微信不显示</span></label>
								</span>
							</td>
						</tr>
						<tr>
			...
			<div class="col-auto">
				...
				<div class="table_full">
					<table width="100%">
						...
						<tr>
							<th width="80">摘要 </th>
							<td><textarea name='post[post_excerpt]' id='description' style='width:98%;height:70px;'  ><?php echo ($post["post_excerpt"]); ?></textarea> </td>
						</tr>
						<tr>
							<th width="80">内容</th>
							<td><span class="must_red">*</span><div id='content_tip'></div>
								...
								<script type="text/javascript"  src="/myweb/static/js/ueditor/editor_config.js"></script>
								<script type="text/javascript"  src="/myweb/static/js/ueditor/editor_all_min.js"></script>
								<script type="text/javascript">
									var editorcontent;
	...
	<script type="text/javascript" src="/myweb/static/js/content_addtop.js<?php echo ($js_debug); ?>"></script>
	<script type="text/javascript">
									$(function() {
