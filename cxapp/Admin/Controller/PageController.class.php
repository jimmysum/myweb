...
class PageController extends \Admin\Controller\AdminController {
	 ...
	 * @param type $map
	 */
	protected function _index_filter(&$map = array()) {
		$map['post_type'] = 2;
		$map['post_status'] = 1;
		$fields = array(
		...
		if (IS_POST) {
			foreach ($fields as $param => $val) {
				if (isset($_POST[$param]) && !empty($_POST[$param])) {
					...
					array_push($map, "$field $operator '$get'");
				}
			}
		} else {
			foreach ($fields as $param => $val) {
	...
	function indexss() {
		...
		$count = D("Posts")->where($where)->count();
		$page = $this->page($count, 20);

		$posts = D("Posts")->where($where)->limit($page->firstRow . ',' . $page->listRows)->select();
	...
	public function edit() {
		...
		if (IS_POST) {
			$_POST['post']['smeta'] = json_encode($_POST['smeta']);
			$result = D("Posts")->save($_POST['post']);
			if ($result) {
				//
				$this->success("保存成功！");
	...
	function delete() {
		...
		if (isset($_POST['ids'])) {
			...
			if (D("Posts")->where("ID in ($ids)")->save($data)) {
				$this->success("删除成功！");
			} else {
				$this->error("删除失败！");
			}
