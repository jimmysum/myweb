...
class PostController extends \Admin\Controller\AdminController {
	...
	function add() {
		...
		if (IS_POST) {
			...
			$result = D("Post")->add($_POST['post']);
			if ($result) {
				$_POST['term']['object_id'] = $result;
				$result = D("TermRelationships")->add($_POST['term']);
				if ($result) {
					$this->success("新增成功！");
				} else {
					$this->error("归类失败！");
			...
			$term = D("Terms")->where("term_id=$term_id")->find();
	...
	public function edit() {
		...
		if (IS_POST) {
			$_POST['smeta']['thumb'] = substr($_POST['smeta']['thumb'], strlen(C("TMPL_PARSE_STRING.__UPLOAD__")));
			$_POST['post']['smeta'] = json_encode($_POST['smeta']);
			$result = D("Post")->save($_POST['post']);
			//echo(D("Post")->getLastSql());die;
			if (false !== $result) {
				$this->success("保存成功！");
			} else {
				$this->error("保存失败！");
	...
	public function listorders() {
		$status = parent::listorders(D("TermRelationships"));
		if ($status) {
			$this->success("排序更新成功！");
		} else {
			$this->error("排序更新失败！");
	...
	function delete() {
		if (isset($_GET['tid'])) {
			$tid = (int) I("get.tid");
			$data['status'] = 0;
			if (D("TermRelationships")->where("tid=$tid")->save($data)) {
				$this->success("删除成功！");
			} else {
				$this->error("删除失败！");
			}
		...
		if (isset($_POST['ids'])) {
			$tids = join(",", $_POST['ids']);
			$data['status'] = 0;
			if (D("TermRelationships")->where("tid in ($tids)")->save($data)) {
				$this->success("删除成功！");
			} else {
				$this->error("删除失败！");
			}
	...
	function check() {
		if (isset($_POST['ids']) && $_GET["check"]) {
			...
			$tids = join(",", $_POST['ids']);
			$objectids = D("TermRelationships")->field("object_id")->where("tid in ($tids)")->select();
			$IDs = array();
	...
	function move() {
		if (IS_POST) {
			if (isset($_GET['ids']) && isset($_POST['term_id'])) {
				$tids = $_GET['ids'];
				if (D("TermRelationships")->where("tid in ($tids)")->save($_POST)) {
					$this->success("移动成功！");
				} else {
					$this->error("移动失败！");
				}
	...
	function clean() {
		if (isset($_POST['ids'])) {
			...
			$status = D("TermRelationships")->where("tid in ($tids)")->delete();
			if ($status) {
				$status = D("Post")->where("ID in ($ids)")->delete();
			}
			...
			if (isset($_GET['id'])) {
				$id = (int) I("get.id");
				$tid = (int) I("get.tid");
				$status = D("TermRelationships")->where("tid = $tid")->delete();
				if ($status) {
					$status = D("Post")->where("ID=$id")->delete();
				}
				if ($status) {
					$this->success("删除成功！");
				} else {
	...
	function restore() {
		if (isset($_GET['id'])) {
			...
			if (D("TermRelationships")->save($data)) {
				$this->success("还原成功！");
			} else {
				$this->error("还原失败！");
			}
